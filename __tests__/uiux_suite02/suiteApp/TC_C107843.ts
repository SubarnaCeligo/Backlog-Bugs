...
test.describe("TC_C107843_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY-MM-DD hh-mm AM/PM)", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("@Zephyr-T7801 @Env-All @Priority-P2 TC_C107843_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY-MM-DD hh-mm AM/PM) UI_Backlog", async ({ io, page }, testInfo) => {
        ...
        await test.step("*** Creating PageProcessor ***", async () => {
            ...
            await io.homePage.fill(selectors.flowBuilderPagePO.SOURCE_MAPPING_PLACEHOLDER, '2023-12-28 04-12 AM')
            ...
            await io.assert.verifyElementContainsText(selectors.basePagePO.RESULT_PREVIEW_CONTENT, "{  \"CreatedDate\": \"2023-12-28 04-12     AM\"}")
