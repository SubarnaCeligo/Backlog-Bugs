...
test.describe("TC_C107894_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (MM/DD/YYYY hh-mm AM/PM)", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("@Zephyr-T7832 @Env-All @Priority-P2 TC_C107894_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (MM/DD/YYYY hh-mm AM/PM) UI_Backlog", async ({ io, page }, testInfo) => {
        ...
        await test.step("*** Creating PageProcessor ***", async () => {
            ...
            await io.homePage.fill(selectors.flowBuilderPagePO.SOURCE_MAPPING_PLACEHOLDER, '05/06/2013 11-12 PM')
            ...
            await io.assert.verifyElementContainsText(selectors.basePagePO.RESULT_PREVIEW_CONTENT, "{  \"CreatedDate\": \"05/06/2013 11-12     PM\"}")
