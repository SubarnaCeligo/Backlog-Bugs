...
test.describe("TC_C107881_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY/M/D hh:mm AM/PM)", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("TC_C107881_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY/M/D hh:mm AM/PM) UI_Backlog", async ({ io, page }, testInfo) => {
        ...
        await test.step("*** Creating PageProcessor ***", async () => {
            ...
            await io.homePage.fill(selectors.flowBuilderPagePO.SOURCE_MAPPING_PLACEHOLDER, '2023/5/2 01:12 PM')
            ...
            await io.assert.verifyElementContainsText(selectors.basePagePO.RESULT_PREVIEW_CONTENT, "{  \"CreatedDate\": \"2023/5/2 01:12     PM\"}")
