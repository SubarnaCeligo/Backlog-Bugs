...
test.describe("TC_C107884_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY/M/D hh:mm (24 hours))", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("@Zephyr-T7822 @Env-All @Priority-P2 TC_C107884_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY/M/D hh:mm (24 hours)) UI_Backlog", async ({ io, page }, testInfo) => {
        ...
        await test.step("*** Creating PageProcessor ***", async () => {
            ...
            await io.homePage.fill(selectors.flowBuilderPagePO.SOURCE_MAPPING_PLACEHOLDER, '2023-5-2 13:12 (24 hours)')
            ...
            await io.assert.verifyElementContainsText(selectors.basePagePO.RESULT_PREVIEW_CONTENT, "{  \"CreatedDate\": \"2023-5-2 13:12 (24 hours)\"}")
