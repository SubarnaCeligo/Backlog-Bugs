...
test.describe("C61932 App crash should not happen if we change IO url something like 'https://qa.staging.integrator.io///[https://qaprod.staging.integrator.io] ' based on the current environment", () => {
  test("C61932 App crash should not happen if we change IO url something like 'https://qa.staging.integrator.io///[https://qaprod.staging.integrator.io] ' based on the current environment", async ({io, page}) => {
      ...
      await io.assert.verifyElementIsDisplayed(selectors.myAccountPagePO.EMAILINPUT, 'Email input is not displayed after sign out');
