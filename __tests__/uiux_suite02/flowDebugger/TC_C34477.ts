...
import C34477 from "@testData/FlowDebugger/C34489.json";
...
test.describe("C34477 - verify the request logs in a list are sorted by timestamp in descending order", () => {
  let id;
  ...
  test("C34477 - verify the request logs in a list are sorted by timestamp in descending order", async ({
    io,
    page
  }) => {
    id = await io.createResourceFromAPI(C34477, "FLOWS");
