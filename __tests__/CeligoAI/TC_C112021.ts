...
test.describe("C112021 Verify Application Should Not Crashes Upon CeligoAI Prompt Closure", () => {
  test("C112021 Verify Application Should Not Crashes Upon CeligoAI Prompt Closure", async ({ io, page }) => {
    ...
    await io.flowBuilder.fill(selectors.connectionsPagePO.CONNECTIONS_DROPDOWN, 'AMAZON REDSHIFT CONNECTION');
    await io.flowBuilder.clickByText('AMAZON REDSHIFT CONNECTION');
    await io.flowBuilder.click(selectors.basePagePO.SAVE);
    await io.flowBuilder.fill(selectors.basePagePO.INPUT_NAME_SELECTOR, 'Amazon Redshift AI');
    ...
    await expect(
      page.locator(selectors.flowBuilderPagePO.OPENAI.PROGRESS_BAR,)).toBeHidden({ timeout: 30000 });
      await io.flowBuilder.loadingTime();
    await io.assert.verifyElementIsDisplayed(
