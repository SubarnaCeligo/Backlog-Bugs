...
test.describe("C119633 Verify the Celigo AI functionality working fine for Postgre SQL", () => {
  test("C119633 Verify the Celigo AI functionality working fine for Postgre SQL", async ({io, page}) => {
    await io.homePage.navigateTo(process.env["IO_Integration_URL"]);
    await io.homePage.click(selectors.flowBuilderPagePO.CREATEFLOW);
    await io.flowBuilder.click(selectors.flowBuilderPagePO.ADD_SOURCE);
    await io.flowBuilder.fill(selectors.settingsPagePO.APP_NAME_INPUT, 'PostgreSQL');
    await io.flowBuilder.click(selectors.flowBuilderPagePO.POSTGRESQL_APPLICATION);
    await io.flowBuilder.click(selectors.connectionsPagePO.CONNECTIONS_DROPDOWN);
    await io.flowBuilder.fill(selectors.connectionsPagePO.CONNECTIONS_DROPDOWN,'POSTGRESQL CONNECTION');
    await io.flowBuilder.clickByText('POSTGRESQL CONNECTION');
    await io.flowBuilder.click(selectors.basePagePO.SAVE);
    await io.flowBuilder.fill(selectors.basePagePO.INPUT_NAME_SELECTOR, 'POSTGRESQL AI');
