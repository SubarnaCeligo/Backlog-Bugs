...
test.describe("C110832 Verify JS Editor is having Celigo AI", () => {
  ...
  test("@Env-QA @Env-IAQA C110832 Verify JS Editor is having Celigo AI", async ({
    io,
    page
  }) => {
    ...
    const isCeligoAINotVisibleInRule = !(await io.flowBuilder.isVisible(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT
    ));
    await io.assert.expectToBeTrue(
      isCeligoAINotVisibleInRule,
      "Celigo AI Not Visible for Rule 1.0 or 2.0"
    );
    ...
    await io.assert.verifyElementAttribute(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_BAR,
      "aria-expanded",
      "false",
      1
    );
    ...
    await expect(
      page.locator(selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT_WINDOW)
    ).toHaveAttribute("data-popper-placement", "top");
    await io.assert.verifyElementContainsText(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT_WINDOW,
      "Provide instructions for Celigo AI to generate a Javascript code for you. "
    );
    await io.assert.verifyElementContainsText(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT_WINDOW,
      "Note: Your instructions will not be saved after you exit the editor window."
    );
    await io.flowBuilder.clickByIndex(
    ...
    const explainDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    ...
    const isCeligoAIThinkingNotVisible = !(await io.flowBuilder.isVisible(
      selectors.flowBuilderPagePO.OPENAI.PROGRESS_BAR
    ));
    await io.assert.expectToBeTrue(
      isCeligoAIThinkingNotVisible,
      "Celigo AI Thinking Not Visible"
    );
    //EXPORT_TRANSFORMATION Continue
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
      "write js code to give current time"
    );
    await io.flowBuilder.loadingTime();
    await page.keyboard.press("Enter");
    await io.assert.verifyElementIsDisplayed(
    ...
    await io.flowBuilder.click(
      selectors.flowBuilderPagePO.OPENAI.JAVASCRIPT_PANEL
    );
    ...
    await expect(
      page.locator(selectors.flowBuilderPagePO.OPENAI.THUMPSUP)
    ...
    const explainExportDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainExportDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
    ...
    const explainHookDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainHookDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    // hover text for the disabled Explain selection button C111482
    await io.flowBuilder.loadingTime();
    let disabledButton = await page.locator(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    ...
    if (buttonBoundingBox) {
      ...
      await expect(
        page
          .locator(
            selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION_HOVER_TEXT
          )
          .first()
      ).toHaveAttribute(
        "aria-label",
        "Select any portion of the code to have Celigo AI generate a human-readable description"
      );
    }
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
    ...
    const explainPlaygroundEnabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainPlaygroundEnabled[0].getAttribute("class")).not.toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.click(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    ...
    const draggableBox = await page.$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION_WINDOW
    );
    if (draggableBox) {
    ...
    await io.flowBuilder.click(selectors.basePagePO.JAVASCRIPTWINDOW);
    const isCeligoAINotVisible = !(await io.flowBuilder.isVisible(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT
    ));
    await io.assert.expectToBeTrue(
      isCeligoAINotVisible,
      "Celigo AI Not Visible for Edit Branching"
    );
    await io.flowBuilder.click(selectors.flowBuilderPagePO.CLOSE_RIGHT_DRAWER);
    ...
    const explainImportFilterDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainImportFilterDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
    ...
    const explainImportDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainImportDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
    ...
    const explainImportPremapDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainImportPremapDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.loadingTime();
    await io.flowBuilder.fill(
    ...
    const explainImportPostmapDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainImportPostmapDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.loadingTime();
    await io.flowBuilder.fill(
    ...
    const explainImportPostSubmitDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(
      await explainImportPostSubmitDisabled[0].getAttribute("class")
    ...
    const explainImportPostResponseDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(
      await explainImportPostResponseDisabled[0].getAttribute("class")
    ).toContain("Mui-disabled");
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
      "write js code to give current time"
    ...
    const explainImportPostAggDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainImportPostAggDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.loadingTime();
    await io.flowBuilder.fill(
    ...
    const explainPlaygroundDisabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainPlaygroundDisabled[0].getAttribute("class")).toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.fill(
    ...
    const errorMsg = page
      .getByText(
        "Enter the function for the Celigo AI to help you in writing the JS code."
      )
      .first();
