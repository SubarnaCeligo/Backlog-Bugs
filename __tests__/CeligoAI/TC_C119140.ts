...
test.describe("C119140 Verify the Reset template,query and script button in AFE windows", () => {
  test.skip("@Env-All C119140  Verify the Reset template,query and script button in AFE windows", async ({
    io,
    page
  }) => {
    ...
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE,
      "Reset query is not displayed"
    );
    ...
    resetQuery.hover();
    const queryHoverText = page
      .getByText(
        "Click or tap to undo all changes and restore the last saved query."
      )
      .first();
    await queryHoverText.waitFor({ state: "visible", timeout: 30000 });
    const resetQueryEnabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE
    );
    expect(await resetQueryEnabled[0].getAttribute("class")).not.toContain(
      "Mui-disabled"
    );
    //C119143 C119144
    ...
    const resetScriptEnabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE
    );
    expect(await resetScriptEnabled[0].getAttribute("class")).not.toContain(
      "Mui-disabled"
    );
    //Verify Reset Script text
    resetScript.hover();
    const scriptHoverText = page
      .getByText(
        "Click or tap to undo all changes and restore the last saved script."
      )
      .first();
    ...
    resetTemplate.hover();
    const tempateHoverText = page
      .getByText(
        "Click or tap to undo all changes and restore the last saved template."
      )
      .first();
    await tempateHoverText.waitFor({ state: "visible", timeout: 30000 });
    const templateQueryEnabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE
    );
    expect(await templateQueryEnabled[0].getAttribute("class")).not.toContain(
      "Mui-disabled"
    );
    //C119144
    await io.flowBuilder.click(selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE);
    ...
    await io.assert.verifyElementDisplayedByText(
      "Reset script",
      'Reset script Label is not displayed"'
    );
    await io.assert.verifyElementAttributeContainsText(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE,
      "class",
    ...
    await io.assert.verifyElementDisplayedByText(
      "Reset query",
      'Reset query Label is not displayed"'
    );
    await io.assert.verifyElementAttributeContainsText(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE,
      "class",
    ...
    await io.assert.verifyElementDisplayedByText(
      "Reset template",
      'Reset template Label is not displayed"'
    );
    await io.assert.verifyElementAttributeContainsText(
      selectors.flowBuilderPagePO.OPENAI.RESET_TEMPLATE,
      "class",
