...
test.describe("C119207 Verify Celigo AI panel enhancements", () => {
  ...
  test("@Env-QA @Zephyr-IO-T18882 C119207 Verify Celigo AI panel enhancements", async ({ io, page }) => {
    ...
    await io.assert.verifyElementIsDisplayed(selectors.flowBuilderPagePO.OPENAI.AICHATBOT_PANEL, "Celigo AI dragbar is not displayed")
    ...
    const updatedClientHeight = await celigoAiWindow.evaluate(node => node.clientHeight);
