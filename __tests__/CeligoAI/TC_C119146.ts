...
test.describe("C119146 Verify the Error Panel in AFE windows", () => {
  test.skip("@Env-QA @Env-IAQA C119146 Verify the Error Panel in AFE windows", async ({
    io,
    page
  }) => {
    ...
    await io.assert.verifyElementIsDisplayed(
      ...
      "Reset template is not displayed"
    );
    ...
    await io.flowBuilder.loadingTime();
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.OPENAI.ARROW_DOWN,
      "arrowDownIcon is not displayed"
    );
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.OPENAI.ERROR_CODEPANEL,
      "codePanel is not displayed"
    );
    //C119146
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.OPENAI.ClOSE_BUTTON,
      "closeIcon is not displayed"
    );
    //C119148
    await io.flowBuilder.click(selectors.flowBuilderPagePO.OPENAI.ARROW_DOWN);
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.OPENAI.ARROW_RIGHT,
      "arrowRightIcon is not displayed"
    );
    await io.flowBuilder.loadingTime();
    const isErrorPanelCollapsed = !(await io.flowBuilder.isVisible(
      selectors.flowBuilderPagePO.OPENAI.ERROR_CODEPANEL
    ));
    await io.assert.expectToBeTrue(
      isErrorPanelCollapsed,
      "Error Panel is not collapsed"
    );
    ...
    await io.assert.verifyElementIsDisplayed(
      selectors.basePagePO.ENABLEAICHATBOT,
      "AI Chatbot is not displayed"
    );
    await io.assert.verifyElementDisplayedByText(
      "AI chatbot",
      'AI Chatbot Label is not displayed"'
    );
  });
});
