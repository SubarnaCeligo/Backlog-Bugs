...
test.describe("C117448 Verify Filter is having Celigo AI", () => {
  ...
  test("@Env-QA @Env-IAQA C117448 Verify Filter is having Celigo AI", async ({
    io,
    page
  }) => {
    ...
    await io.assert.verifyElementContainsText(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT_WINDOW,
      "Provide instructions for Celigo AI to generate a Filter rules for you. "
    );
    await io.assert.verifyElementContainsText(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_HELPTEXT_WINDOW,
    ...
    await io.assert.verifyElementAttribute(
      ...
      "placeholder",
      "Tell me about your filter here... I will apply your request to the existing filter unless you tell me to replace it"
    );
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
    ...
    await io.assert.verifyElementContainsText(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION,
      "Explain filter rules"
    );
    await io.flowBuilder.hover(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_FILTER_RULES,
      0,
      false
    );
    await io.flowBuilder.waitForElementAttached(
      selectors.flowBuilderPagePO.RUNTEST_TOOLTIP
    );
    await io.assert.verifyElementIsDisplayed(
      selectors.flowBuilderPagePO.RUNTEST_TOOLTIP,
      "Tooltip for explanation button is not displayed"
    );
    ...
    await io.flowBuilder.loadingTime();
    const promptErrorMsg = page
      .getByText("Invalid prompt - Please provide relevant prompt.")
      .first();
    await promptErrorMsg.waitFor({ state: "visible", timeout: 30000 });
    await io.flowBuilder.loadingTime();

    (
      await page.$(selectors.flowBuilderPagePO.EM2DOT0PO.ACE_EDITOR_INPUT)
    ).focus();
    ...
    await io.flowBuilder.loadingTime();
    await expect(
      page.locator(selectors.flowBuilderPagePO.AFE_RESULT_PANEL)
    ).toContainText("FALSE:");

    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
      "remove previous filter and add following filters  i. brand is Apple ii. price is greater than 500 iii. category is smartphones"
    );
    await io.flowBuilder.loadingTime();
    await page.keyboard.press("Enter");
    await io.assert.verifyElementIsDisplayed(
    ...
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
      "filter by where item quantity is 2, amount is less than 160 and customerId is C12345"
    );
    await io.flowBuilder.loadingTime();
    await page.keyboard.press("Enter");
    await io.assert.verifyElementIsDisplayed(
    ...
    await io.flowBuilder.fill(
      selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD,
      "update the filter with quantity to be 1 instead"
    );
    await io.flowBuilder.loadingTime();
    await page.keyboard.press("Enter");
    await io.assert.verifyElementIsDisplayed(
    ...
    //C117473
    await io.flowBuilder.loadingTime();
    const explainEnabled = await page.$$(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
    );
    expect(await explainEnabled[0].getAttribute("class")).not.toContain(
      "Mui-disabled"
    );
    await io.flowBuilder.click(
      selectors.flowBuilderPagePO.OPENAI.EXPLAIN_SELECTION
