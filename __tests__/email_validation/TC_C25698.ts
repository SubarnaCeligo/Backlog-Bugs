...
test.describe("C25698 Verify the link on the email on resolved errors", () => {
  ...
  test("C25698 Verify the link on the email on resolved errors", async ({ io, page }) => {
    ...
    await io.homePage.waitForElementAttached('[data-test="account-dashboard-open-errors"]');
    await io.flowBuilder.click('[data-test="account-dashboard-open-errors"]');
    ...
    console.log('resoonse from mail', res);
