...
import TC_C22280 from "@testData/EM2.0/TC_C22280_Verify_TraceKey_Error_Details_Value_1.json";

test.describe("TC_C22280", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Beginning of Test Suite ***", async ()=>{});
    await io.goToFlowsPage();
  });
  test("@Env-All @Zephyr-IO-T6241 TC_C22280| TraceKey at level 2 of the traversal in the form of a leaf node", async ({io,page}, testInfo) => {
    await io.goToFlowsPage();
    //*Create Flows
    var flows = await io.api.createImpOrExpAndFlowsThruAPI(TC_C22280);
    var flowId = flows.get(TC_C22280.name)["flowId"];
await test.step(
      "Created Flow " +
        flows.get(TC_C22280.name)["flowName"] +
        " With ID " +
        flows.get(TC_C22280.name)["flowId"],async () => {
    ...
    await io.api.checkJobStatusFromAPI(
      TC_C22280.name,
      flows.get(TC_C22280.name)["flowId"],
      [0, 0, 1]
    );
    //Opening error table
