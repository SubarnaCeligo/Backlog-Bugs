...
import flowbranch from "@testData/flowbranching/TC_C47546.json";

test.describe("@Author-ParthPatel TC_C47546 | Golden", () => {
  let flowId;
  ...
  test("@Env-All @Zephyr-IO-T17628 TC_C47546_Retry_Error_Zendesk_Export", async ({io,page}, testInfo) => {
    ...
    await io.flowbranching.flowBranchingPage.decreaseDrawer();
    expect(errors.includes("4 errors")).toEqual(true);
    await io.homePage.loadingTime();
    await io.flowbranching.flowBranchingPage.fitScreenViewInFlowBranch();
