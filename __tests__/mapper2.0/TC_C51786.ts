...
import TC from "@testData/Mapper2.0/TC_C51786.json";
import { ElementHandle } from "playwright/test";

test.describe("TC_C51786", () => {
  test("@Env-All @Zephyr-IO-T22439 TC_C51786 | Verify If the below Source Data data types are mapped to the 'Number' destination data type then below validations should be shown", async ({io,page}, testInfo) => {
    ...
    await io.assert.expectToContainValue("Mapper 2.0: data: You can't map object (source) to number (destination)", res, "");
    await test.step("*** Mapper 2.0: data: You can't map object (source) to number (destination) ***", async ()=>{});

    await io.assert.expectToContainValue("Mapper 2.0: new_variant: You can't map [number] (source) to number (destination)", res, "");
    await test.step("*** Mapper 2.0: new_variant: You can't map [number] (source) to number (destination) ***", async ()=>{});

    await io.assert.expectToContainValue("Mapper 2.0: taxable: You can't map boolean (source) to number (destination)", res, "");
    await test.step("*** Mapper 2.0: taxable: You can't map boolean (source) to number (destination) ***", async ()=>{});

    await io.assert.expectToContainValue("Mapper 2.0: values: You can't map [string] (source) to number (destination)", res, "");
    await test.step("*** Mapper 2.0: values: You can't map [string] (source) to number (destination) ***", async ()=>{});

    await io.assert.expectToContainValue("Mapper 2.0: images: You can't map [object] (source) to number (destination)", res, "");
    await test.step("*** Mapper 2.0: images: You can't map [object] (source) to number (destination) ***", async ()=>{});

    await io.assert.expectToContainValue("Mapper 2.0: requires_shipping: You can't map [boolean] (source) to number (destination)", res, "");
    await test.step("*** Mapper 2.0: requires_shipping: You can't map [boolean] (source) to number (destination) ***", async ()=>{});
  });
});
