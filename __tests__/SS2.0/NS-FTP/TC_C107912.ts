...
test.describe("SS2.0 Flows", () => {
  ...
  test("TC_C107916_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (D-MONTH-YYYY hh:mm (24 hours))", async ({
    io,
    page
  }, testInfo) => {
    ...
    await test.step("*** Enable and run the Flow *** ", async () => {
      ...
      await io.api.validateJobCountFromAPI(
        testInfo.title,
        NS_FTP.qa__expectedDashboardCount
      );
    });
  });
});
