...
test.describe("TC_C21262_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY-M-D hh:mm AM/PM)", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("TC_C21262_Verify syncing dateTime fields in NS SS2.x import flows based on Date format added in the settings (YYYY-M-D hh:mm AM/PM) UI_Backlog", async ({ io, page }, testInfo) => {
