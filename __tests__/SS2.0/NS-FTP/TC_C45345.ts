...
test.describe("TC_C45345_Verify the 'help text' and 'field labels' inside the settings drawer where ever the 'Source Field 'and 'Destination field ' text will exists", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("TC_C45345_Verify the 'help text' and 'field labels' inside the settings drawer where ever the 'Source Field 'and 'Destination field ' text will exists UI_Backlog", async ({ io, page }, testInfo) => {
