...
test.describe("T28742_T28743_T28737_T28736 Verify the license Entitlements notification(Free tier,Standard,professional ,Enterprise)", () => {
  ...
  test("@Zephyr-IO-T28735 @Env-QA @Priority-P2 Verify the license Entitlements notification whenever the endpoints and flow usage has been exceeded for free tier", async ({
    ...
    await io.api.putCall(
      `v1/test/licenses/${platformLicense._id}`,
      {...getLicensePayload(platformLicense),expires: "2044-04-10T13:14:33.363Z",tier: 'free', numEndpoints: 1,"disableOverage": false}
    );
    await io.myAccountPage.navigateTo(io.data.links.MY_ACCOUNT_PAGE_URL);
    await io.myAccountPage.click(selectors.myAccountPagePO.SUBSCRIPTION);
    await page.waitForLoadState();
    await test.step("C28735 Verify the license Entitlements notification whenever the endpoints and flow usage has been exceeded for professional tie", async () => {
      await io.assert.verifyElementContainsText('[id="notification"]', 'Your account has exceeded its entitlements. Request an upgrade.');
  }); 
  await io.connectionPage.clickByText("Request an upgrade.");   
  await io.assert.verifyElementIsDisplayed(selectors.homePagePO.DIALOG, "We will contact you to discuss your business needs and recommend an ideal subscription plan.");
