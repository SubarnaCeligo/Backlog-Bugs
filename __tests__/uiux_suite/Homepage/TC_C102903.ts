...
test.describe("C102903 Verify User is able to navigate to correct window on clicking any option from nav bar", () => {
  test("C102903 Verify User is able to navigate to correct window on clicking any option from nav bar", async ({
    io,
    page
  }) => {
    await io.homePage.navigateTo(process.env["IO_UI_CONNECTOR_URL"] + "home");
    await page.hover('[data-test="Resources"]');
    await io.homePage.addStep("Hovered over 'Resources'");
    await io.homePage.click('[data-test="Imports"]');
