...
test.describe("C60401 Verify whether data types(select,integer) are supporting for path parameters in 2.0(ex:ADP) for resource type Talent and api endpoint Get specific talent of a worker", () => {
    test("C60401 Verify whether data types(select,integer) are supporting for path parameters in 2.0(ex:ADP) for resource type Talent and api endpoint Get specific talent of a worker", async ({io, page}) => {
