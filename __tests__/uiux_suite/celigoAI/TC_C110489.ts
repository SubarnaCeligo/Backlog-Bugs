...
test.describe("C110489 Verify system generates the Big query after the user has provided the prompt", () => {
  test("C110489 Verify system generates the Big query after the user has provided the prompt", async ({io, page}) => {
    ...
    await io.flowBuilder.fill(selectors.connectionsPagePO.CONNECTIONS_DROPDOWN,'BIGQUERY CONNECTION');
    await io.flowBuilder.clickByText('BIGQUERY CONNECTION');
    await io.flowBuilder.click(selectors.basePagePO.SAVE);
    await io.flowBuilder.fill(selectors.basePagePO.INPUT_NAME_SELECTOR, 'Google BigQuery AI');
    ...
    await io.flowBuilder.fill(selectors.flowBuilderPagePO.OPENAI.CELIGO_AI_FIELD, 'Get the customer details from gptverse');
