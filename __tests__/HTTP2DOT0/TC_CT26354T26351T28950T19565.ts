...
test("@Env-All CT26354 Verify configure refresh token is enabled", async ({
    ...
    await io.flowBuilder.waitForElementAttached(selectors.flowBuilderPagePO.TABLEAU);
    await io.flowBuilder.loadingTime();
    ...
    await io.flowBuilder.loadingTime();
    await io.assert.expectToBeTrue(await (await page.$(selectors.connectionsPagePO.REFRESH_TOKEN_CUSTOM)).isChecked(), "Configure refreshtoken is not checked");
...
test("@Env-All T28950 Verify success and error values are saved when media type is plain text", async ({
    ...
    await io.flowBuilder.click("[data-test='Non-standard API response patterns']");
    await io.flowBuilder.loadingTime();
    await io.flowBuilder.fill(
      "[name='/http/auth/failValues']","500"
    );
    ...
    await io.flowBuilder.fill(
      "[name='/http/ping/failValues']","500"
    );
    await io.flowBuilder.fill(
      "[name='/http/ping/successValues']","200"
    );
    await io.flowBuilder.fill(
    ...
    const ele = await (await page.$("[name='/http/auth/failValues']")).getAttribute("value")
    ...
    await io.flowBuilder.click(selectors.connectionsPagePO.HOW_TO_TESTCONNECTION);
    const errorvalue = await (await page.$("[name='/http/ping/failValues']")).getAttribute("value")
    await io.assert.expectToContainValue(
      '500',
      errorvalue,
      'error value is not displayed'
    );
    const successvalue = await (await page.$("[name='/http/ping/successValues']")).getAttribute("value")
    await io.assert.expectToContainValue(
      '200',
      successvalue,
 ...
 await io.flowBuilder.click(selectors.flowBuilderPagePO.CLOSE);
    ...
    await io.homePage.fill(selectors.connectionsPagePO.CONNECTION_PAGE_SEARCH_BAR, "HTTP PLAINTEXT");
...
test("@Env-All T19565 Verify xml file in AFE", async ({
    io,
    page
  }) => { await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);;
  await io.flowBuilder.loadingTime();
  ...
  ConnName[1].fill("HTTP XML Parser Connection");
  await io.flowBuilder.fill(selectors.importPagePO.HTTP_BASEURI, "https://5634b9e3-4bde-425a-adba-c5a031019b7a.mock.pstmn.io");
  ...
  await io.homePage.loadingTime();
  await io.flowBuilder.waitForElementAttached(selectors.flowBuilderPagePO.RESOUCEPATH);

  await io.flowBuilder.fill(selectors.flowBuilderPagePO.RESOUCEPATH, "/soap:Envelope/soap:Body/AuthenticationTokenGetResponse");
  await io.flowBuilder.click(selectors.basePagePO.SAVE_AND_CLOSE);
  await io.homePage.loadingTime();
  await io.homePage.loadingTime();
  await io.flowBuilder.click(selectors.flowBuilderPagePO.ADD_DESTINATION_OR_LOOKUP);
  ...
  await io.homePage.waitForElementAttached(selectors.basePagePO.TOOLS);
    ...
    await io.flowBuilder.loadingTime();
    await io.homePage.fill(selectors.connectionsPagePO.CONNECTION_PAGE_SEARCH_BAR, "HTTP XML parser");
    await io.homePage.loadingTime()
    await io.homePage.click(selectors.integrationPagePO.OPENACTIONSMENU);
    await io.homePage.loadingTime();
