...
test.describe("TC_T26975_Test to validate that user is able to see array of objects format on parsed output section when user specify the relative path on 'Path to records in HTTP response body' field if array of array structure is not there at the top level", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });
    test.skip("TC_T26975 @Zephyr-IO-T26975 @Env-All @Priority-P2", async ({ io, page }) => {
        ...
        await io.exportsPage.fill(selectors.importPagePO.PATH_TO_RECORDS_IN_HTTP_RESPONSE_BODY_HTTP,"values");
        await io.homePage.addStep("*** Filled path to response body field ***");
