...
test.describe("T22231 - Verify in the Oracle DB export, Group data dropdown should have suggested list of fields only if records are returned in preview and no suggestions if no preview data is returned", () => {
  test("@Zephyr-IO-T22231 @Env-QA @Epic-IO-35739 @Priority-P2 - Verify in the Oracle DB export, Group data dropdown should have suggested list of fields only if records are returned in preview and no suggestions if no preview data is returned", async ({ io, page }) => {
