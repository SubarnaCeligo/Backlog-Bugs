...
test.describe(`TC_IO-T26981  Test to validate that user is seeing below help text when we click on icon "Check this box if the first record returned in the HTTP response body contains the headers for all the subsequent records returned in the HTTP response body." http export/lookup`, () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });

    test("IOT26980", async ({ io }) => {
        ...
        await io.assert.verifyElementDisplayedByText('Check this box if the first record returned in the HTTP response body contains the headers for all the subsequent records returned in the HTTP response body.','Help text is incorrect');
