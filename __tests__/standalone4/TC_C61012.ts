...
test.describe("TC_C61012_Verifying the prompt when SuiteBundle Suitescript 1.0 is selected while creating a new NS export and when only suiteapp is installed", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Beginning of Test Suite ***", async ()=>{});
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("TC_C61012 @Env-All @Zephyr-IO-T23049", async ({io,page}, testInfo) => {
