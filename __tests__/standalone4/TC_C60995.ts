...
test.describe("TC_C60995_Verify suiteapp install prompt while creating new NS export and no suiteapp or suitebundle are installed", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Beginning of Test Suite ***", async ()=>{});
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("TC_C60995 @Env-All @Zephyr-IO-T23032", async ({io,page}, testInfo) => {
    ...
    test.step("*** Verifying the text content in the prompt displayed ***", async ()=>{});
  });
});
