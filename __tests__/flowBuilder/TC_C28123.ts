...
import TC from "@testData/FlowBuilder/TC_C28123.json";

test.describe("TC_C28123 | Verify Leave pop dialog when closing the drawer with unsaved changes", () => {
  let flowId;
  ...
  test("@Env-All @Zephyr-IO-T2841| Verify on Edit retry data", async ({ io, page }) => {
    ...
    expect(await page.locator(selectors.basePagePO.DISCARD_CHANGES).isVisible()).toBeTruthy();
  })

  test("@Env-All @Zephyr-IO-T2841| Verify on Settings and Launch form builder under settings", async ({ io, page }) => {
    await io.api.createImpOrExpAndFlowsThruAPI(TC);
    await io.homePage.loadingTime();
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
  ...
  test("@Env-All @Zephyr-IO-T2841| Verify on Flow schedule and Flow schedule override", async ({ io, page }) => {
    await io.api.createImpOrExpAndFlowsThruAPI(TC);
    await io.homePage.loadingTime();
    flowId = await io.api.getFlowId(TC.name);
  ...
  test("@Env-All @Zephyr-IO-T2841| Verify on Input filter, Mappings, Proceed on failure and Results mappings ", async ({ io, page }) => {
    await io.api.createImpOrExpAndFlowsThruAPI(TC);
    await io.homePage.loadingTime();
    flowId = await io.api.getFlowId(TC.name);
    await io.flowBuilderDashboard.navigateToFlowBuilderInEM2(flowId);
  ...
  test("@Env-All @Zephyr-IO-T2841| Verify on Scripts", async ({ io, page }) => {
    await io.api.createImpOrExpAndFlowsThruAPI(TC);
    await io.homePage.loadingTime();
    flowId = await io.api.getFlowId(TC.name);
    await io.flowBuilderDashboard.navigateToFlowBuilderInEM2(flowId);
  ...
  test("@Env-All @Zephyr-IO-T2841| Verify Export and Import Form builders", async ({ io, page }) => {
    await io.api.createImpOrExpAndFlowsThruAPI(TC);
    await io.homePage.loadingTime();
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
    await io.homePage.click(selectors.basePagePO.ACCOUNT_BUTTON);
