...
import TC_C27346 from "@testData/FlowBuilder/TC_C27346.json";

test.describe("TC_C27346", () => {
  let flowId;
  ...
  test("@Env-All @Zephyr-IO-T2827|Verify child record Tracekey is overidden with the Override child record trace key template value when given any valid handlebar expression - PP lookup", async ({io,page}) => {
    await io.api.createImpOrExpAndFlowsThruAPI(TC_C27346);
    flowId = await io.api.getFlowId(TC_C27346.name);
