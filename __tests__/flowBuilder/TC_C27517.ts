...
import TC_C27517 from "@testData/FlowBuilder/TC_C27517.json";

test.describe("TC_C27517", () => {
  let flowId;
  ...
  test("@Env-All @Zephyr-IO-T2833|Verify child record Tracekey is overidden with the Override child record trace key template value when given field reference - PP import", async ({io,page}) => {
    //Create Flows
    await io.api.createImpOrExpAndFlowsThruAPI(TC_C27517);
    var flowId = await io.api.getFlowId(TC_C27517.name);
    //Run Flow
    await io.flowBuilderDashboard.navigateToFlowBuilderInFB(flowId);
    await io.homePage.loadingTime();await io.homePage.loadingTime();
    ...
    await io.homePage.fill('[name="/traceKeyTemplate"]','{{record._PARENT.user.id}}');
