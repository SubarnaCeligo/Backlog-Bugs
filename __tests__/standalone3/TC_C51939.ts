...
import TC_C51939 from "@testData/STANDALONE/TC_C51939.json";

test.describe("TC_C51939", () => {
  let flowId: string;
  ...
  test("@Zephyr-IO-T26380 @Env-All TC_C51939", async ({io,page}, testInfo) => {
    test.step("*** Go to flows page ***", async ()=>{});
    await io.goToFlowsPage();
    var flows = await io.api.createImpOrExpAndFlowsThruAPI(TC_C51939);
    await test.step("Created Flow " + flows.get(TC_C51939.name)["flowName"],async ()=>{});
    flowId = flows.get(TC_C51939.name).flowId;
