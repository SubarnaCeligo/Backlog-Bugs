...
import { encrypt} from "@celigo/aut-utilities";

test.describe("TC_C22440", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Beginning of Test Suite ***", async ()=>{});
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-All @Zephyr-IO-T5839 TC_C22440", async ({io,page}, testInfo) => {
    ...
    await io.homePage.fill(selectors.connectionsPagePO.NAME_INPUT, "TC_C22440_Token_regenerate_validationoperation");
    ...
    await io.homePage.fillWebPage(selectors.flowBuilderPagePO.SEARCHBUTTON,"TC_C22440_Token_regenerate_validationoperation");
    await page.waitForTimeout(1000);
    test.step("*** Searching for created API Token ***", async ()=>{});
    ...
    await page.waitForTimeout(5000);
    test.step("*** Clicked On generate new API Token***", async ()=>{});

    test.step("*** Getting request data form NS connection response***", async ()=>{});

    const encryptedToken = encrypt("Bearer " + tokencopied);
    var responseData1 = await io.api.getCall(endpoint, undefined, encryptedToken);
    await expect(responseData1).toBeUndefined();
