...
test.describe("C22352 Verify UX for download integration dropdown field", () => {
    test("C22352 Verify UX for download integration dropdown field", async ({io, page}) => {
        ...
        expect( await page.$(selectors.homePagePO.CLONE_INTEGRATION)).not.toBe(null);
        expect( await page.$(selectors.homePagePO.GENERATE_TEMPLATE_ZIP)).not.toBe(null);
        expect( await page.$(selectors.homePagePO.DELETE_INTEGRATION)).not.toBe(null);
    });
});
