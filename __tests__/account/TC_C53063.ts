...
test.describe("C53063 Verify that the 30 and 60 days option is enable for license types Professional tire", () => {
  ...
  test("@Env-All @Zephyr-IO-T14552 @Zephyr-IO-T28437 C53063 Verify that the 30 and 60 days option is enable for license types Professional tire", async ({
    io,
    page
  }) => {
    ...
    if (signInLink.endsWith("signin")) {
      ...
      if (rateLimitErrorsVisibility) {
        let rateLimitErrors = await page.locator(selectors.importPagePO.RATE_LIMIT_ERRORS_MESSAGE).textContent();
