...
import FTP from "@testData/STANDALONE/TC_C33381_Verify_DBQuery_DynamoDB_Output_Premap_Hook.json";

test.describe("TC_C33381_Verify_DBQuery_DynamoDB_Output_Premap_Hook", () => {
  let flowID;
  ...
  test("@Env-All @Zephyr-IO-T5538 TC_C33381_Verify_DBQuery_DynamoDB_Output_Premap_Hook", async ({io,page}, testInfo) => {
    ...
    test.step("*** Clicking on Dynamo DB Import button ***", async ()=>{});
    await io.homePage.click(selectors.flowBuilderPagePO.IMPORT);

    await io.homePage.loadingTime();
    await io.homePage.isPageReady();

    test.step("*** Clicking on DynamoDB Query handlebar ***", async ()=>{});
    await io.homePage.click( selectors.importPagePO.DYNAMODBQUERY);
   
    await io.homePage.loadingTime();
    await io.homePage.isPageReady();

    test.step("*** Verifying DynamoDB Query data is Output of Premapscript applied", async ()=>{});
    ...
    test.step("*** Clicking on DynamoDB Query Handlebar close button ***", async ()=>{});
