...
import FTP from "@testData/STANDALONE/TC_C33407_Verify_Directory_GdriveImport_output_Premap_Hook.json";

test.describe("TC_C33407_Verify_Directory_GdriveImport_output_Premap_Hook", () => { 
  let flowID;
  ...
  test("@Env-All @Zephyr-IO-T13717 TC_C33407_Verify_Directory_GdriveImport_output_Premap_Hook", async ({io,page}, testInfo) => {
    ...
    test.step("*** Clicking on Gdrive Import button ***", async ()=>{});
    await io.homePage.clickButtonByIndex(selectors.importPagePO.IMPORT_TRANSFER, 1);

    await io.homePage.loadingTime();
    await io.homePage.isPageReady();

    test.step("*** Clicking on Directory path handlebar ***", async ()=>{});
    ...
    test.step("*** Clicking on Directory path Handlebar close button ***", async ()=>{});
