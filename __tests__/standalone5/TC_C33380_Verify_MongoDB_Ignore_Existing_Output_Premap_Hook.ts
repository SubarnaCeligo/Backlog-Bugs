...
import FTP from "@testData/STANDALONE/TC_C33380_Verify_MongoDB_Ignore_Existing_Output_Premap_Hook.json";

test.describe("TC_C33380_Verify_MONGODB_Ignore_Existing_Output_Premap_Hook", () => {
  let flowID;
  ...
  test("@Env-All @Zephyr-IO-T5537 TC_C33380_Verify_MongoDB_Ignore_Existing_Output_Premap_Hook", async ({io,page}, testInfo) => {
    ...
    await io.homePage.loadingTime();
    await io.homePage.isPageReady();
    test.step("*** Clicking on MONGODB Import button ***", async ()=>{});
    await io.homePage.click(selectors.flowBuilderPagePO.IMPORT);
    await io.homePage.loadingTime();
    await io.homePage.isPageReady();
    test.step("*** Clicking on MONGODB Document handlebar ***", async ()=>{});
    await io.homePage.click(selectors.importPagePO.MONGODBDOCUMENT);
    test.step("*** Verifying MONGODB Document with ignore Existing filter Input data is Output of Premapscript applied", async ()=>{});
    await io.homePage.loadingTime();
    ...
    test.step("*** Clicking on MONGODB Document Handlebar close button ***", async ()=>{});
