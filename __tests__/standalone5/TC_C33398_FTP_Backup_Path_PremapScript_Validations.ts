...
import FTP from "@testData/STANDALONE/TC_C33398_FTP_Backup_Path_PremapScript_Validations.json";

test.describe("TC_C33398_FTP_Backup_Path_PremapScript_Validations", () => {
  let flowID;
  ...
  test("@Env-All @Zephyr-IO-T5552 TC_C33398_FTP_Backup_Path_PremapScript_Validations", async ({io,page}, testInfo) => {
    ...
    await io.homePage.loadingTime();
    await io.homePage.isPageReady();
    test.step("*** Clicking on FTP Backup Path handlebar ***", async ()=>{});
    ...
    test.step("*** Verifying FTP Backup Path Input data is Output of Premapscript applied", async ()=>{});
    await io.homePage.loadingTime();
    ...
    test.step("*** Clicking on FTP Backup Path Handlebar close button ***", async ()=>{});
