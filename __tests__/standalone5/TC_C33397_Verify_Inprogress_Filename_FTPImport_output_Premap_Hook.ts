...
import FTP from "@testData/STANDALONE/TC_C33397_Verify_Inprogress_Filename_FTPImport_output_Premap_Hook.json";

test.describe("TC_C33397_Verify_Inprogress_Filename_FTPImport_output_Premap_Hook", () => {
  let flowID;
  ...
  test("@Env-All @Zephyr-IO-T5551 TC_C33397_Verify_Inprogress_Filename_FTPImport_output_Premap_Hook", async ({io,page}, testInfo) => {
    ...
    test.step("*** Clicking on Use Temp file checkbox ***", async ()=>{});
    await io.homePage.click(selectors.flowBuilderPagePO.FILE_INPROGRESS);
    await io.homePage.loadingTime();
    

    test.step("*** Clicking on In Progress File Handlebar ***", async ()=>{});
    await io.homePage.clickButtonByIndex(selectors.flowBuilderPagePO.INPROGRESS_HANDLEBAR, 1);

    await io.homePage.loadingTime();
    await io.homePage.isPageReady();

    test.step("*** Verifying In progress Filename Input data is Output of Premapscript applied", async ()=>{});
