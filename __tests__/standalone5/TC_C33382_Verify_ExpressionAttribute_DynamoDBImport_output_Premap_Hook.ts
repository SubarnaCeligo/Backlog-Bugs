...
import FTP from "@testData/STANDALONE/TC_C33382_Verify_ExpressionAttribute_DynamoDBImport_output_Premap_Hook.json";

test.describe("TC_C33382_Verify_ExpressionAttribute_DynamoDBImport_output_Premap_Hook", () => {
  let flowID;
  ...
  test("@Env-All @Zephyr-IO-T5539 TC_C33382_Verify_ExpressionAttribute_DynamoDBImport_output_Premap_Hook", async ({io,page}, testInfo) => {
    ...
    test.step("*** Clicking on DynamoDB Import button ***", async ()=>{});
    await io.homePage.click(selectors.flowBuilderPagePO.IMPORT);

    await io.homePage.loadingTime();
    await io.homePage.isPageReady();

    test.step("*** Clicking on ExpressionAttribute Values  handlebar ***", async ()=>{});
    await io.homePage.click("[data-test='dynamodb.expressionAttributeValues']");
    ...
    test.step("*** Clicking on ExpressionAttribute Values Handlebar close button ***", async ()=>{});
