...
test.describe("TC_C25554", () => {
  ...
  test("TC_C25554", async ({io,page}, testInfo) => {
    ...
    var result1 = await io.homePage.getTextFromElement(selectors.basePagePO.SNACKBAR, "New accounts must be attached to a valid business email address.");
    await await io.assert.expectToBeTrue(result1, "");
    await io.homePage.click(selectors.myAccountPagePO.ERRCLOSE);
    await test.step("*** Verified User Error Shown For Invalid Email ***",()=>{});
    await io.homePage.clearTextValue(selectors.loginPagePO.EMAIL);
