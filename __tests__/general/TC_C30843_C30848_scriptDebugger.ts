...
test.describe("TC_C30843_C30848_scriptDebugger", () => {
  test.beforeEach(async ({io}) => {
    ...
    if(!postSubmit)
      postSubmit = await io.api.createScriptViaAPI( FTP.scriptBody);
  ...
  test("TC_C30843_C30848_scriptDebugger", async ({io,page}, testInfo) => {
    ...
    await io.homePage.click(selectors.homePagePO.SCRIPTDEBUGGER);
    ...
    await io.homePage.isPageLoaded();
    await io.homePage.isPageReady();
    let text = selectors.myAccountPagePO.FETCHEXCUTIONTEXT
    await await io.assert.expectToContainValue(text, "Fetching paused... ", "");
    ...
    await io.homePage.isPageLoaded();
    await io.homePage.isPageReady();
    let text1 = selectors.myAccountPagePO.FETCHEXCUTIONTEXT
    await await io.assert.expectToContainValue(text1, "Fetching logs... ", "");
