...
import TC_C22321 from "@testData/GENERAL/TC_C22321.json";

test.describe("TC_C22321", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    await test.step("*** Beginning of Test Suite ***",()=>{});
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Zephyr-IO-T2311 @Env-All  TC_C22321", async ({io,page}, testInfo) => {
    await io.goToFlowsPage();
    //*Create Flows
    var flows = await io.api.createImpOrExpAndFlowsThruAPI(TC_C22321);
    var flow = await io.api.getFlowId(TC_C22321.name);
    //Checking job status
    await io.api.checkJobStatusFromAPI(
      TC_C22321.name,
      flows.get(TC_C22321.name)["flowId"],
      [0, 0, 1]
    );

    await io.flowBuilderDashboard.navigateToFlowBuilderInEM2( flows.get(TC_C22321.name)["flowId"])
