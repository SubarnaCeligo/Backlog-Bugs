...
test.describe("TC_C32543", () => {
  test("TC_C32543", async ({io,page}, testInfo) => {
    ...
    await await io.assert.expectToContainValue(actualMsg, "TC_C32543| completed 0 0 0", "");
    await test.step("*** verified there should be one export job completed with 0 success, 0 errors, 0 ignore, and 0 pages ***",()=>{});
    await test.step("Verified the performance when the page generators has 0 records and multiple page processors",()=>{});
  });
});
