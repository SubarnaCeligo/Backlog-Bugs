...
test.describe("TC_T37418_TC_T37420_Test to validate text after the cursor is retained when we select any helper from dropdown and there is no space between cursor and text && Test to validate the characters used for filtering are not repeated and selected text is visible when we select any helper from dropdown and there is no space between cursor and text", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });
    test("@Zephyr-IO-T37418 @Zephyr-IO-T37420 @Env-All @Priority-P2", async ({ io, page }) => {
        ...
        await io.assert.verifyElementContainsText(selectors.flowBuilderPagePO.HANDLEBAR_CONTENT, '"ad{{abs age}}dResult": {{add id age}}');
        await io.homePage.addStep("*** Validating that The text towards the right is not wiped out ***");
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });
});