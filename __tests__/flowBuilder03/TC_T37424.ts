...
test.describe("TC_T37424_Test to validate closing brackets are automatically added if user is at end of expression , when we select any helper from dropdown and there is no space between cursor and text", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });
    test("@Zephyr-IO-T37424 @Env-All @Priority-P2", async ({ io, page }) => {
