...
test.describe("TC_T37423_Test to validate duplicate closing brackets are not added if we already have closing brackets when we select any field from dropdown and there is no space between cursor and text", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });
    test("@Zephyr-IO-T37423 @Env-All @Priority-P2", async ({ io, page }) => {
