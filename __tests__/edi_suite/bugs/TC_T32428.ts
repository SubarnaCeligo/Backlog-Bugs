...
test.describe("@Author-Shriti S TC_T32428-Verify that standalone listeners are shown while creating FTP EDI X12 standalone flows", () => {
  ...
  test("@Bug-IO-79115 @Env-All @Priority-P2 @Zephyr-IO-T32428 Verify that standalone listeners are shown while creating FTP EDI X12 standalone flows", async ({ io, page }) => {
    ...
    await io.homePage.fill(selectors.integrationPagePO.INTEGRATION_PAGE_SEARCH_BAR, "Standalone flows");
    await io.homePage.clickByTextByIndex('Standalone flows', 0);
    ...
    await io.assert.expectNotToBeNull(selectedListener, 'Listener is not showing in standalone tile.');
    await io.assert.expectNotToBeValue(selectedListener, 'Please select','Listener is not showing in standalone tile.' );
