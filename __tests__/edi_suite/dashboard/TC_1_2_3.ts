...
test.describe("Verify all the elements of EDI Dashboard", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("Verify all the elements of EDI Dashboard", async ({ io, page }) => {
    //Go to Dashboard
    await io.myAccountPage.navigateTo(process.env["IO_UI_CONNECTOR_URL"] + "dashboard");

    //Click on EDI Activity
    await io.homePage.addStep("Verify that 'EDI activity' tab is added to IO Dashboard");
