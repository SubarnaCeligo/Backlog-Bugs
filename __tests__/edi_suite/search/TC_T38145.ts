...
test.describe("@Author-Shriti S Verify that pagination is respected for search results", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-QA @Env-IAQA @Epic-IO-85710 @Priority-P2 @Zephyr-IO-T38145 Verify that pagination is respected for search results", async ({ io, page }) => {
    ...
    //Before search
    await io.assert.verifyElementIsDisplayed('[value="100"]', 'Default pagination not displayed');
    ...
    await io.assert.expectToBeTrue(allMatch, 'Search is not working');
