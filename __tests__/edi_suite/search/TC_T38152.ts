...
test.describe("@Author-Shriti S Verify UI behaviour when only consecutive spaces are given as search term", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-QA @Env-IAQA @Epic-IO-85710 @Priority-P2 @Zephyr-IO-T38152 Verify UI behaviour when only consecutive spaces are given as search term", async ({ io, page }) => {
    ...
    await io.homePage.loadingTime();
    await io.assert.verifyElementDisplayedByText("Must be between 3 and 30 characters", 'Message is not displayed');
