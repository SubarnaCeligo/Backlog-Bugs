...
test.describe("@Author-Shriti S Verify that only 30 characters are allowed in the search term", () => {
  ...
  test("@Env-QA @Env-IAQA @Epic-IO-85710 @Priority-P2 @Zephyr-IO-T38143 Verify that only 30 characters are allowed in the search term", async ({ io, page }) => {
    ...
    await io.homePage.fill(selectors.dashboardPagePO.EDI_SEARCH_BOX, "*abc*");
    await page.keyboard.press('Enter');
    await io.homePage.loadingTime();
    await io.assert.verifyElementDisplayedByText("Query can contain only 1 wildcard (*), with at least 3 characters preceding", 'Error message is not displayed');
