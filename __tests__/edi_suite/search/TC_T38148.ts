...
test.describe("@Author-Shriti S Verify that search term is trimmed if there are leading/trailing spaces", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-QA @Env-IAQA @Epic-IO-85710 @Priority-P2 @Zephyr-IO-T38148 Verify that search term is trimmed if there are leading/trailing spaces", async ({ io, page }) => {
    ...
    await io.assert.expectToBeTrue(allMatch, 'Search term is not trimmed');
  });
});