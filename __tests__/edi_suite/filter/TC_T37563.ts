...
test.describe("@Author-Shriti S Verify that EDI transactions can be filtered using multiple Doc type values", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-QA @Env-IAQA @Epic-IO-89826 @Priority-P2 @Zephyr-IO-T37563 Verify that EDI transactions can be filtered using multiple Doc type values", async ({ io, page }) => {
    ...
    let result = (await io.homePage.getText(selectors.dashboardPagePO.FLOW_NAME)).toString();
    ...
    await io.assert.expectToBeTrue(filterWorking, 'EDI documents are not filtered using Doc type');
  });
});