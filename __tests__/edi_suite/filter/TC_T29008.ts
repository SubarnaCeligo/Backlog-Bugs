...
test.describe("@Author-Shriti S Verify that EDI activity can be filtered using single FA status.", () => {
  ...
  test("@Env-QA @Env-IAQA @Epic-IO-89826 @Priority-P2 @Zephyr-IO-T29008 Verify that EDI activity can be filtered using single FA status.", async ({ io, page }) => {
    ...
    let index = 4;//Value=Not Aplicable
