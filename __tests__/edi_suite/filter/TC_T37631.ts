...
test.describe("@Author-Shriti S Verify that EDI transactions can be filtered using single File type value", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-QA @Env-IAQA @Epic-IO-89826 @Priority-P2 @Zephyr-IO-T37631 Verify that EDI transactions can be filtered using single File type value", async ({ io, page }) => {
    ...
    let result = (await io.homePage.getText(selectors.dashboardPagePO.FILETYPE_COLUMN)).toString();
