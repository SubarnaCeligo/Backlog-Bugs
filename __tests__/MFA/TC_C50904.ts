...
test.describe("C50904 C99342 Verify the validation message if user wants to to select both SSO & MFA", () => {
  test("@Env-All @Zephyr-IO-T19653 @Zephyr-IO-T25378 C50904 C99342 Verify the validation message if user wants to to select both SSO & MFA", async ({
    ...
    await io.flowBuilder.hover(selectors.myAccountPagePO.REQUIRE_SSO_TOGGLE);

    //check if the error message is displayed
    const hoverText = (
      await io.myAccountPage.getText(
