...
test.describe("C50905 Verify that only the owner/admin should be able to define the number of days for a trusted device.", () => {
  test("@Env-QA @Env-IAQA C50905 Verify that only the owner/admin should be able to define the number of days for a trusted device.", async ({io, page}) => {
    ...
    await page.locator('Button.MuiButton-disableElevation').last().click()
