...
import TC_C22286 from "@testData/EM2.0/TC_C22286.json";

test.describe("TC_C22286", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Beginning of Test Suite ***", async ()=>{});
    await io.goToFlowsPage();
  });
  test("@Env-All @Zephyr-IO-T6244 TC_C22286| TraceKey at both level 1 and level 2 of the traversal", async ({io,page}, testInfo) => {
    await io.goToFlowsPage();
    //*Create Flows
    var flows = await io.api.createImpOrExpAndFlowsThruAPI(TC_C22286);
    var flowId = flows.get(TC_C22286.name)["flowId"];
await test.step(
      "Created Flow " +
        flows.get(TC_C22286.name)["flowName"] +
        " With ID " +
        flows.get(TC_C22286.name)["flowId"],async () => {
    ...
    await io.api.checkJobStatusFromAPI(
      TC_C22286.name,
      flows.get(TC_C22286.name)["flowId"],
      [0, 0, 1]
    );
    //Opening error table
