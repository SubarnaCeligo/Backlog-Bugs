...
import TC_C22281 from "@testData/EM2.0/TC_C22281.json";

test.describe("TC_C22281", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Beginning of Test Suite ***", async ()=>{});
    await io.goToFlowsPage();
  });
  test("@Env-All @Zephyr-IO-T6242 TC_C22281| TraceKey at level 1 of the traversal in the form of an internal node", async ({io,page}, testInfo) => {
    await io.goToFlowsPage();
    //*Create Flows
    var flows = await io.api.createImpOrExpAndFlowsThruAPI(TC_C22281);
    var flowId = flows.get(TC_C22281.name)["flowId"];
await test.step(
      "Created Flow " +
        flows.get(TC_C22281.name)["flowName"] +
        " With ID " +
        flows.get(TC_C22281.name)["flowId"],async () => {
    ...
    await io.api.checkJobStatusFromAPI(
      TC_C22281.name,
      flows.get(TC_C22281.name)["flowId"],
      [0, 0, 1]
    );
    //Opening error table
