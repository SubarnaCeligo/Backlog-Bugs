...
test.describe("TC_C29043_Filtering_completed_flows_date_Range_validation", () => {
  test.beforeEach(async ({io,page}, testInfo) => {
    test.step("*** Navigate to Home Page ***", async ()=>{});
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
  });
  test("@Env-All @Zephyr-IO-T6438 TC_C29043_Filtering_completed_flows_date_Range_validation", async ({io,page}, testInfo) => {
...
await test.step(
      "*** Filtering the date range woth 30 days  ***"
, async ()=>{});
    await io.homePage.click(
      await "//p[text()='Last 30 days']"
    );
    await io.homePage.click(
      await selectors.myAccountPagePO.APPLY_BUTTON
    );

    test.step("*** Validating the last 30 days flows  ***", async ()=>{});
    var data = (await io.homePage.getText("//p[text()=' Completed date range:']/../button")).toString();
    await io.assert.expectToContainValue("Last 30 days",data, "");
