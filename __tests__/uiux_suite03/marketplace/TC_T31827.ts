...
test.describe("@Author-Shriti S Verify the search results and app counts message with a search term that matches only templates.", () => {
  ...
  test("@Env-QA @Epic-IO-51028 @Priority-P2 @Zephyr-IO-T31827 Verify the search results and app counts message with a search term that matches only templates.", async ({ io, page }) => {
    ...
    await io.marketplacePage.fill(selectors.marketplacePagePO.SEARCH_MARKETPLACE, "mailchimp");
    ...
    await io.assert.expectToContainValue('template(s)', message, '');
    await io.assert.expectNotToContainValue("Search results for 'mailchimp'", message, '');
