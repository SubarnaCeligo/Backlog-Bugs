...
test.describe("@Author-Shriti S Verify the help message when an application name is searched for which we have a prebuilt connector but we don’t have any marketplace templates or apps.", () => {
  ...
  test("@Env-QA @Epic-IO-51028 @Priority-P2 @Zephyr-IO-T31830 Verify the help message when an application name is searched for which we have a prebuilt connector but we don’t have any marketplace templates or apps.", async ({ io, page }) => {
    ...
    await io.marketplacePage.fill(selectors.marketplacePagePO.SEARCH_MARKETPLACE, "alibaba");
    ...
    await io.assert.expectToContainValue("We could not find anything related to 'alibaba'", message, '');
    await io.assert.verifyElementDisplayedByText("Don't see integration apps and templates related to alibaba ? Integrate with our prebuilt connector(s).", 'Invalid message');
