...
test.describe("T24284 - Verify user can create export/lookup/import with the 'Use same flow step' radio button after updating the configuration.", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
        //make sure the account has at least one shopify export
        // await io.createResourceFromAPI(exportJson, 'EXPORT');
    });
    test("@Epic-IO-54539 @Priority-P2 T24284 - Verify user can create export/lookup/import with the 'Use same flow step' radio button after updating the configuration.", async ({ io, page }) => {
