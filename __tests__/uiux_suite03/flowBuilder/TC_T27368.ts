...
test.describe("T27368 Check for NoSqlDbs if the mapper 2.0 is enabled and getting saved and persisted ", () => {
    ...
    test("Check for NoSqlDbs if the mapper 2.0 is enabled and getting saved and persisted MongoDB", async ({ io, page }) => {
        await io.homePage.addStep("*** starting test case for mongoDb ***");
        ...
        await io.homePage.addStep("*** end test case for mongoDb ***");
    });
    test("Check for NoSqlDbs if the mapper 2.0 is enabled and getting saved and persisted DynamoDB", async ({ io, page }) => {
