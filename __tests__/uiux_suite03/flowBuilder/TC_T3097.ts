...
test.describe("T3097 Custom settings should not be Shrinked once we select script and save in the Form builder", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);

  });
  test("T3097 Custom settings should not be Shrinked once we select script and save in the Form builder", async ({ io, page }) => {
