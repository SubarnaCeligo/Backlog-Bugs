...
test.describe("C68515 Next Integration flow set-Verify set Next integration flow run successfully when the current Branching flow gets run on scheduled day/time", () => {
    test("C68515 Next Integration flow set-Verify set Next integration flow run successfully when the current Branching flow gets run on scheduled day/time", async ({io, page}) => {
      ...
      await io.homePage.navigateTo(process.env["IO_Integration_URL"]);
      await io.flowBuilder.fill(selectors.flowBuilderPagePO.SEARCH, 'Branching2_Scheduling_DND');
      ...
      await io.flowBuilder.fill(selectors.flowBuilderPagePO.SEARCH, 'Branching_Scheduling3_DND');
      await io.flowBuilder.waitForElementAttached('text="In progress..."');
      await io.assert.verifyElementDisplayedByText("In progress...", 'The flow scheduled did not run');
      await io.flowBuilder.clickByText('Branching2_Scheduling_DND');
      await io.flowBuilder.click(selectors.flowBuilderPagePO.SCHEDULE_FLOW);
