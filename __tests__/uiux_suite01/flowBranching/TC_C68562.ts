...
test.describe(`C68562 Verify user is upload the integration zip file having Multiple linear flows in the template and able to install the template`, () => {
  ...
  test.afterEach(async ({ io }) => {
    ...
    await io.homePage.fill(selectors.basePagePO.SEARCH_RECYCLEBIN,"C68562_Automation" );
  ...
  test(`@Env-All @Zephyr-IO-T17463 C68562 Verify user is upload the integration zip file having Multiple linear flows in the template and able to install the template`, async ({
    io,
    page
  }) => {
    await io.homePage.goToMenu("Resources", "Templates");
    ...
    await io.homePage.waitForElementAttached(
      `:has-text("C68562_Automation") ${selectors.flowBuilderPagePO.OPEN_ACTIONS_MENU}`
    );
    await io.homePage.click(
      `tbody tr:has-text("C68562_Automation") ${selectors.flowBuilderPagePO.OPEN_ACTIONS_MENU}`
    );
    await io.homePage.click('[data-test="uploadTemplateZip"]');
    const fileInput = await page.$(selectors.basePagePO.UPLOAD_FILE);
    await fileInput.setInputFiles("testData/inputData/Templates/C68562_NEW.zip");
    await io.homePage.loadingTime();
