...
test.describe("TC_T29693_T29696_Test to validate that user is able to run a flow with Azure synapse export", () => {
    ...
    test("@Epic-IO-65860 @Priority-P2 @Zephyr-IO-T29693 @Zephyr-IO-T29696 @Env-All", async ({ io, page }) => {
        ...
        await io.homePage.fill(selectors.integrationPagePO.INTEGRATION_PAGE_SEARCH_BAR, "synapse_export_flow_run_DND");
        await io.homePage.addStep("*** Searched for the flow ***");
        await io.flowBuilder.loadingTime();
        await io.homePage.clickByText("Azure_synapse_export_flow_run_DND");
