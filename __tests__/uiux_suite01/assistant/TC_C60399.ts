...
test.describe(`C60399 Verify whether data types(select,integer) are supporting for path parameters in 2.0(ex:ADP) for resource type time and api endpoint timeoff`, () => {
  test(`@Env-QA C60399 Verify whether data types(select,integer) are supporting for path parameters in 2.0(ex:ADP) for resource type time and api endpoint timeoff`, async ({
    io,
    page
  }) => {
