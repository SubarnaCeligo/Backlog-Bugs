...
test.describe("T4548 Verify that the test ping is successfull for quick base connection ,when valid details are given to validate the conenction", () => {
  test.beforeEach(async ({ io }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);

  });
  test("T4548 Verify that the test ping is successfull for quick base connection ,when valid details are given to validate the conenction", async ({ io, page }) => {
