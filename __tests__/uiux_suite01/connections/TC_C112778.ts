...
test.describe("C112778 - Verify the 'Create destination / lookup'(export bubble) page on flow builder screen when the search criteria does not yield any results.", () => {
    test.beforeEach(async ({ io }) => {
        await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
    });
    test("C112778 - Verify the 'Create destination / lookup'(export bubble) page on flow builder screen when the search criteria does not yield any results.", async ({ io, page }) => {
