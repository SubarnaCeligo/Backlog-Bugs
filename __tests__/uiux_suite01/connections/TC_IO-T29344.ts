...
test.describe('Test to validate user is able to see the "Azure Synapse" logo at top right corner', () => {
  test("@Env-QA @Zephyr-IO-T29344 @Priority-P2 This is to verify the Azure Synapse logo beside connection name after clicking create connection" , async ({
    io,
    page
  }) => {
  ...
  test("@Env-QA @Zephyr-IO-T29344 @Priority-P2 'Azure Synapse' logo at top right corner ", async ({
    io,
    page
  }) => {
    await io.myAccountPage.navigateTo(io.data.links.HOME_PAGE_URL);
