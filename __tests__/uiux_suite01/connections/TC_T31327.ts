...
test.describe("T31327 Verify if the SSL certificate and SSL client key by providing different formats when SSL certificate type is PEM format", () => {
  ...
  test("@Env-All @Zephyr-IO-T31328 @Priority-P2 T31327 Verify if the SSL certificate and SSL client key by providing different formats when SSL certificate type is PEM format UI_Backlog", async ({ io, page }) => {
    ...
    certFile = await page.$(`${selectors.connectionsPagePO.SSL_CLIENT_CERT_CONTAINER} ${selectors.basePagePO.UPLOAD_FILE}`);
    keyFile = await page.$(`${selectors.connectionsPagePO.SSL_CLIENT_KEY_CONTAINER} ${selectors.basePagePO.UPLOAD_FILE}`);
    ...
    await io.connectionPage.addStep("*** Validating that connection is saved by checkking if the save button is disbled ***"); 
    SAVE_BUTTON = page.locator(selectors.basePagePO.SAVE);
    expect(SAVE_BUTTON).toHaveAttribute("disabled");
