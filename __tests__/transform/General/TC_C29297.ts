...
test.describe("TC_C29297", () => {
  ...
  test("TC_C29297", async ({io,page}, testInfo) => {
    ...
    await io.homePage.click(
      selectors.flowBuilderPagePO.VERIFICATION
    );
    await io.homePage.click("[data-value='basic']");
    await io.homePage.fillWebPage(
      selectors.flowBuilderPagePO.WEBHOOKUSERNAME,
      "test"
    );
    await io.homePage.fillWebPage(
      selectors.flowBuilderPagePO.WEBHOOKPASSWORD,
      "gfhgf"
    );
    await io.homePage.click(
      "//button[@aria-label='Generate URL']"
    );
    await test.step(" Clicking on Generate URL button",()=>{});
    await io.homePage.isPageLoaded();
    await io.homePage.isPageReady();
    await io.homePage.click(
      selectors.flowBuilderPagePO.COPYCLIPBOARD
    );
...
await test.step(
      "Verified the Generate token button is replaced with icon button with tool tip"
, async ()=>{});
    await io.homePage.navigateTo(io.data.links.HOME_PAGE_URL);
